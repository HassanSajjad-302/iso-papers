...
This results in better compilation speed.
...
By making compiler a shared library,
we can achieve faster build speeds.

## Why is this faster

i) API allows the build system to intercept the files read by the compiler.
So, the compilation of multiple compilation units can use the same cached file.
...
(scanning time + compilation time) per compilation unit for LLVM is ```8.46%```.
...
iii) Estimated average scanning time for the project LLVM was ```217.9ms```.
...
source files.
If LLVM is built with C++20 modules, there will be ```2854 + 3598``` process launches
instead of ```2854```.
...
That is because more ```compilerState``` and the cached files need to be kept in the memory.

However, this can be alleviated by the following:

Few build systems generally group files together in a ```target```.
...
This way only the ```compileState``` of the files of the dependency target need to be kept
in the memory.
```compilerState``` of the files of the dependent target only come in the picture once
...
Because all such compilations can use one cached read instead of reading themselves.

## Consequences of this choice for the current build systems
...
Plans for supporting this in the build system HMake are described here:
...
Instead of an executable, the compiler could be a shared-library.
Or there could be a separate driver supporting the functionality described here.
Compilation pause and resume capability needs to be built into the compiler.
HMake loads this driver dll or the compiler dll.
Now it calls the following symbols ```newCompile``` and ```resumeCompile```.
...
{
    // if (!completed && !errorOccurred), then pointer to the compiler state to be preserved by the build-system, else
    // nullptr
    void *compilerState;
...
The compiler calls ```newCompile``` function passing it the
compile-command for the module file.
The compile command, however, does not include any dependencies.
If the compiler sees an import of a module, it sets the ```objectFileOrModuleName``` String
of the ```CompileOutput``` return value.
Same is the case for header-units but as mentioned in declaration,
ifcFileOrHeaderUnitPath is used.
The build-system now will preserve the ```compilerState``` and
...
the build-system will call ```resumeCompile``` function passing it the ifcFile.
```resumeCompile``` is called until file has no dependency not provided and the compilation
succeeds.
If only the ifcFile is returned and no objectFile on compilation completion,
the build-system assumes that the compiler is using 2-phase model.
In this case,
on a different thread it will call ```getObjectFile``` to get objectFile.
...
The argument ```getFileContents``` is used by the compiler to get file contents of any file
...
- "Declarations of new names should have an introducer like most other places."
  ...
  Some people found it surprising that that would introduce a new name, as most
...
When `match` is executed, the expression to the left is evaluated and matched
...
> | `[` _designator_~0~ `:` _pattern_~0~ `,` _designator_~1~ `:` _pattern_~1~ `,` ... `,` _designator_~N~ : _pattern_~
> N~ `]`
...
*initializer* `;`]{.rm}
> |     [*attribute-specifier-seq~opt~* *decl-specifier-seq* *ref-qualifier~opt~* *structured-binding-pattern*
*initializer* `;`]{.add}
